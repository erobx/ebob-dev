...
<svelte:fragment slot="trail">
    ...
    <div class="md:hidden relative">
        ...
        <div id="dropdown" class="absolute right-0 mt-2 w-48 bg-dark-blue text-white rounded-lg shadow-lg hidden">
            <a href="/" class="block px-4 py-2 text-sm text-[#9E9F90]" on:click={closeDropdown}>Home</a>
            <a href="/about" class="block px-4 py-2 text-sm text-[#9E9F90]" on:click={closeDropdown}>About</a>
            <a href="/projects" class="block px-4 py-2 text-sm text-[#9E9F90]" on:click={closeDropdown}>Projects</a>
        </div>
    </div>
